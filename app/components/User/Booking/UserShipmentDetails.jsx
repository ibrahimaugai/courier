...
export default function UserShipmentDetails({
  ...
  const activeCities = useMemo(() => {
    ...
    // If filtered list is empty but total list is not, return all cities as fallback
  ...
  const availableServices = useMemo(() => {
    ...
    // If filtered list is empty but total list is not, return all for product as fallback
    return (filtered.length > 0 ? filtered : allForProduct)
      .map(s => ({ value: s.serviceName, label: s.serviceName }))
      .sort((a, b) => a.label.localeCompare(b.label))
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                readOnly
                disabled
                className="w-full px-4 py-2 border border-gray-300 bg-gray-100 text-gray-500 cursor-not-allowed rounded-md focus:outline-none transition-colors"
              />
            </div>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <select
                ...
                onChange={handleInputChange}
                required
                disabled={!formData.product || availableServices.length === 0 || isReadOnly}
