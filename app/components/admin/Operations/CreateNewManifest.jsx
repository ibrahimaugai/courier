...
export default function CreateNewManifest({ setActivePage }) {
  ...
  const fetchDrivers = async () => {
    try {
      const result = await api.getManifestDrivers()
  ...
  const fetchVehicles = async () => {
    try {
      const result = await api.getManifestVehicles()
      const data = Array.isArray(result) ? result : (result?.data || [])
      setVehicles(data)
    } catch (err) {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const payload = {
        manifestCode: formData.manifestCode,
      ...
      await api.createManifest(payload)
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-2xl font-black text-gray-900">Create Manifest</h1>
          </div>
        </div>
        <button
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    type="text"
                    value={formData.manifestCode}
                    onChange={(e) => setFormData(prev => ({ ...prev, manifestCode: e.target.value }))}
                    placeholder="e.g. MENI260000329"
